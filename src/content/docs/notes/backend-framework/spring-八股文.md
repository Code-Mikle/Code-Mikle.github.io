...
RESTful 风格接口，是一种按照“资源”来设计 URL，并使用 HTTP 方法表示操作类型的接口设计方式。
...
URL 里只放资源名词，不放动词。相对资源做什么操作，全靠 HTTP 方法来表达。
...
| 新增用户     | `POST`    | `/users`   |
| 修改指定用户 | `PUT`     | `/users/1` |
| 部分修改用户 | `PATCH`   | `/users/1` |
| 删除指定用户 | `DELETE`  | `/users/1` |
...
> 幂等性：对同一个请求执行一次和执行多次，对服务器资源产生的最终效果相同。

注意，幂等关注的是服务器资源的最终状态，不要求每次返回的 HTTP 状态码和响应内容完全相同。

| HTTP 方法               | 是否幂等   | 原因                                       |
| ----------------------- | ---------- | ------------------------------------------ |
| `GET`（查询资源）       | 是         | 重复查询不会改变资源状态                   |
| `POST`（创建资源）      | 通常不是   | 重复执行可能创建多个资源或重复执行业务     |
| `PUT`（完整修改资源）   | 是         | 多次使用相同内容覆盖同一资源，最终状态相同 |
| `PATCH`（部分修改资源） | 默认不保证 | 多次执行局部修改，结果可能不断累积         |
| `DELETE`（删除资源）    | 是         | 删除一次或多次，最终资源都处于不存在状态   |

**常见状态码：**

RESTful 接口通常配合 HTTP 状态码表示结果：

```
200 OK：请求成功  -  GET 请求成功返回数据
201 Created：创建成功  -  POST 创建资源成功
204 No Content：成功但没有响应体  -  常用于删除成功，也可用于修改成功但不需要返回数据的情况
400 Bad Request：请求格式或参数错误  -  比如：JSON 格式错误、必填参数缺失、参数类型错误等
401 Unauthorized：未认证  -  没有携带 Token、Token 已过期、没登录等
403 Forbidden：无权限  -  用户身份已确认，但没有权限执行该操作
404 Not Found：资源不存在  -  请求的资源找不到
500 Internal Server Error：服务器内部发生了未预期异常  -  空指针异常、SQL 执行异常等
```
...
`@Component` 将普通类注册为 Spring Bean，任何类都可以用。后面三个是它的语义化衍生注解。本质其实都一样，区分的目的是为了代码可读性，一眼能够看出来这个类属于哪一层。
...
`@Service` 用于业务层，本质上是特殊的 `@Component`。
...
`@Repository` 用于数据访问层，可以将部分数据库异常转换为 Spring 的统一异常。
...
`@Controller` 用于 Spring MVC 控制层，通常返回页面。
...
`@RestController` 用于 REST 接口，相当于 `@Controller + @ResponseBody` ，自动为所有 `@RequestMapping` 方法添加 `@ResponseBody` 语义 → 方法返回值直接写入 HTTP 响应体（Response Body），无需手动加 `@ResponseBody`。
...
通常用于注册第三方类，或者需要手动创建的对象。

`@ComponentScan` 指定 Spring 扫描 Bean 的包。
...
`@Autowired` 是 Spring 定义的注解，默认按照类型注入 Bean，如果类型不唯一，再按名称找。
...
同时，也可以显式使用 `@Qualifier` 在存在多个相同类型 Bean 的情况下，显式指定注入哪个 Bean。
...
`@Primary` 存在多个相同类型 Bean 时，将某个 Bean 设置为默认选择。
...
`@Resource` 是 Java 定义的注解，先按照名称注入，也可以指定 Bean 名称，名称查不到，再根据类型查找。

```java
@Service
public class UserServiceImpl implements UserService {

    // 方式1：按名称注入（推荐显式指定 name）
    @Resource(name = "userMapper")
    private UserMapper userMapper;

    // 方式2：按类型注入（当容器中该类型唯一时）
    @Resource
    private PasswordEncoder passwordEncoder; // Spring Boot 自动配置的 Bean

    // 方式3：字段 + name + type 混合（name 优先）
...
```java
@Value("${server.port}")
private int port; // 获取 application.yml 等配置文件中的 port 内容
```
...
prototype：每次获取创建一个实例
request：每个 HTTP 请求一个实例
session：每个 Session 一个实例
```

`@Lazy` 让 Spring 在真正需要某个 Bean 时才去创建它。也可以在配置文件中设置 `spring.main.lazy-initialization=true` 来全局开启。
...
也可以用于延迟注入，辅助打破部分循环依赖：

```java
public AService(@Lazy BService bService) {...}
```

`@PostConstruct` Bean 依赖注入完成后执行初始化方法。
...
`@RequestMapping` 配置请求路径和请求方式。一般用于为 Controller 下所有接口设置公共路径前缀。
...
请求：`GET /users/10`，此时 `id` 为 `10`。

`@RequestParam` 获取查询参数或表单参数。即将 HTTP 请求中的查询参数（Query Parameter） 绑定到控制器方法的参数上。
...
请求：`GET /users?name=张三&tagNameList=程序员&tagNameList=Java&tagNameList=爱运动`

`@RequestBody` 将 JSON 请求体自动反序列化为 Java 对象。
...
`@ResponseBody` 将方法返回值写入 HTTP 响应体，通常转换成 JSON。
...
`@CrossOrigin` 为该 Controller（或其方法）启用跨域请求支持，解决浏览器同源策略（Same-Origin Policy）拦截问题。

例如前端运行在：`http://localhost:5173`，后端运行在：`http://localhost:8080`。协议、域名或端口不同，就属于不同的 Origin，浏览器可能阻止前端直接访问后端接口。

可以在 Controller 上配置：

```java
@CrossOrigin(origins = "http://localhost:5173") // 允许哪个源（协议+域名+端口）跨域访问，也就是说允许http://localhost:5173 发送到后端的请求
@RestController
@RequestMapping("/users")
public class UserController {...}
```

当浏览器发请求到 `/user/login` 时，Spring 自动添加，然后浏览器接收到的响应头：
...
表示该 Controller 下的接口允许来自 `http://localhost:5173` 的跨域请求。也可以放在单个方法上：
...
实际项目中，如果大量接口都需要跨域，通常采用全局配置，而不是在每个 Controller 上重复写：
...
`@Valid` 触发 Jakarta Bean Validation 参数校验。
...
这些主要属于 Jakarta Validation，并不是 Spring 自己定义的注解。

`@Validated` Spring 提供的校验注解，支持分组校验，也可以用于方法参数校验。
...
`@ControllerAdvice` 对多个 Controller 进行统一处理，常用于全局异常处理。
...
`@RestControllerAdvice` 相当于 `@ControllerAdvice` + `@ResponseBody`，常用于 REST 接口的全局异常处理。
...
`@Transactional` 声明事务，但是不能够解决并发问题
...
```java
@Scheduled(cron = "0 0 2 * * ?") // 秒 分 时 日 月 星期
public void clearData() {...}
```

在配置类上通常需要开启：`@EnableScheduling`，一旦加上这个注解，Spring 就会去扫描项目中所有使用了 `@Scheduled` 注解的方法。

缓存注解

```
@Cacheable：优先读取缓存，没有时执行方法并写入缓存
@CachePut：执行方法并更新缓存
@CacheEvict：删除缓存
```
...
通常需要开启：`@EnableCaching`

### Spring Boot 常用注解

`@SpringBootApplication` Spring Boot 启动类的核心注解。
...
```
@SpringBootConfiguration 告诉 Spring 当前类是配置类
@EnableAutoConfiguration 自动配置
@ComponentScan 组件扫描
```

`@ConfigurationProperties` 将一组配置绑定到 Java 对象。
...
`@OpenAPIDefinition` 这是 Knife4j / SpringDoc（Swagger 的升级版）提供的注解，用于生成在线 API 文档。

`@Data` 这是 Lombok 注解，自动生成类对应的 getter 和 setter 方法。

`@Slf4j` 也是 Lombok 注解，用于自动创建 SLF4J 日志对象。常见日志级别如下：
...
## Spring、Spring MVC、Spring Boot 之间的区别与联系？

```
Spring (Spring Framework)：基础框架
Spring MVC：Spring 中负责 Web 开发的模块
Spring Boot：简化 Spring 项目搭建和配置的工具
```

**Spring** 通常指 Spring Framework。它提供最基础的企业级开发能力，例如：
...
**Spring MVC** 是 Spring Framework 中专门用于 Web 开发的模块，主要负责：
...
**Spring Boot** 是对 Spring Framework 的工程化封装，主要解决传统 Spring 项目配置复杂的问题。它提供：
