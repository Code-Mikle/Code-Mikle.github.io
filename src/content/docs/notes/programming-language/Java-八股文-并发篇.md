...
JMM（Java Memory Model，Java 内存模型）是 JVM 定义的一套“多线程访问共享内存时应该遵守的规则”。它主要规定了多个线程之间如何读取、修改、同步共享变量，以及一个线程对共享变量的修改什么时候能被另一个线程看到。
...
JMM 抽象出了两个概念：主内存（Main Memory）和工作内存（Working Memory）。如下图所示：

![image-20260826144529040](./assets/image-20260826144529040.png)
...
多线程安全问题主要来自多个线程并发读写访问共享可变数据数据，而线程的执行顺序不可控。从而导致三个安全问题：
