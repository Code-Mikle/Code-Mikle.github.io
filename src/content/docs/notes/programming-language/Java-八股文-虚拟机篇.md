...
Java 的类加载机制，就是 JVM 把 `.class` 字节码从磁盘、网络等外部来源加载进内存，并最终变成可以执行的 `Class` 对象的过程。

类加载的主要过程分为如下步骤：

1. 加载阶段（Loading），找到类文件并读取字节码到内存，在方法区生成类的运行时数据结构，同时在堆里创建一个 Class 对象。
2. 链接阶段（Linking），分为：
   - 验证（Verification），验证字节码格式是否合规，比如：Class 文件格式是否正确、JDK 版本是否兼容、是否存在非法字节码等；
   - 准备（Preparation），给静态变量分配内存并设默认值，比如：`int = 0`、`reference = null` 等；
   - 解析（Resolution），把符号引用（如，类名、方法名）替换为内存中的直接引用（如，指针、偏移量）。
3. 初始化阶段（Initialization），执行类构造器、静态代码块和静态变量赋值，这时候才真正跑用户写的初始化逻辑。
...
方法执行完以后 `user` 这个引用没了。如果没有其他地方保存该对象 `User 对象` 很快就变成垃圾。所以大量对象创建后，使用几毫秒，就死亡了。只有少部分对象会长期存活。这就是所谓弱分代假说（Weak Generational Hypothesis）
...
3. **标记-整理算法（Mark-Compact）**：同样用于**老年代**，老年代对象活得久。它在标记存活对象后，会将它们向内存的一端移动，然后清理剩下的内存。该算法解决了内存碎片问题，但移动对象的成本较高。
