...
> MySQL InnoDB 采用 B+ 树作为普通索引结构，主要是因为数据库按页进行磁盘 I/O，而 B+ 树分叉度高、树高度低，可以减少磁盘 I/O；非叶子节点只保存索引键和子节点指针，因此单页可以容纳更多索引项；同时叶子节点有序并相互连接，既适合等值查询，也非常适合范围查询、排序和顺序扫描。因此 B+ 树在磁盘数据库场景下比二叉平衡树、Hash 和普通 B 树更适合作为通用索引结构。
>
...
数据库中最昂贵的不是比较，而是磁盘 I/O。如果有 1000 万条数据，使用普通二叉搜索树：
...
一棵平衡二叉树大概需要 `log₂(10000000) ≈ 24`。也就是说，最坏可能需要访问二十多层节点。如果每个节点都分散在不同磁盘页上，就意味着可能发生二十多次磁盘 I/O，这个耗时是很长的。
...
- B+ 树的非叶子节点只保存索引键和子节点指针。

  这是 B+ 树相对 B 树的重要优势。假设是 B 树：
...
叶子节点：真正的索引记录
  ...
  因为非叶子节点不存完整数据，所以同样一个 16 KB 页面能保存更多索引键和指针。例如：
  
  ```
  B 树一个节点：[主键 + 完整用户数据 + 指针]
  ```

  用户数据可能很大，一个页只能装几十条。而 B+ 树：`[主键 + 指针]`，可能一个页面就能保存上千个索引项。结果就是单个节点可以拥有更多子节点，从而树的分叉都更高，树的高度也就更低，那么磁盘 I/O 就更少。

- B+ 树特别适合范围查询。叶子节点有序并相互连接，适合范围查询。
...
红黑树也是平衡搜索树，但它本质上仍然是二叉树：每个节点最多两个子节点。因此如果数据非常多：
...
Redo Log，即重做日志，记录数据页发生了哪些修改，崩溃后应该怎样重做。比如：执行某句 SQL 后，记录某个数据页的某个偏移量改成了什么值，这个修改应该怎样重做。
...
Undo Log，即回滚日志，记录的是修改之前的旧版本信息。比如：执行某句 SQL 后，记录修改的值原来是什么，这个修改怎样撤销。

作用：

- 事务回滚

  如果事务执行 ROLLBACK，InnoDB 就根据 Undo Log 逆向撤销修改。
...
Binary Log，记录数据库发生的逻辑操作，也就是原始 SQL 语句或者行变更前后的值。例如：创建表、修改表结构、插入数据、修改数据、删除数据。某句 SQL 执行后，记录某个值被修改为了什么。
