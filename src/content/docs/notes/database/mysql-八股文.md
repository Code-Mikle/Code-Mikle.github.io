...
> **总结**：
>
> MySQL InnoDB 采用 B+ 树作为普通索引结构，主要是因为
>
> 数据库按页进行磁盘 I/O，而 B+ 树分叉度高、树高度低，可以减少磁盘 I/O；
>
> 非叶子节点只保存索引键和子节点指针，因此单页可以容纳更多索引项；
>
> 同时叶子节点有序并相互连接，既适合等值查询，也非常适合范围查询、排序和顺序扫描。
>
> 因此 B+ 树在磁盘数据库场景下比二叉平衡树、Hash 和普通 B 树更适合作为通用索引结构。
...
- B+ 树的非叶子节点只保存索引键和子节点指针。
  ...
  用户数据可能很大，一个页只能装几十条。而 B+ 树：

  ```
  [主键 + 指针]
  ```

  可能一个页面就能保存上千个索引项。结果就是：
...
- B+ 树所有数据都集中在叶子节点，查询性能更加稳定。
  ...
  所以大多数查询都会走 `根节点 → 中间节点 → 叶子节点`，路径长度比较稳定。
...
Binary Log，记录数据库发生逻辑数据变更。例如：创建表、修改表结构、插入数据、修改数据、删除数据。某句 SQL 执行后，记录某个值被修改为了什么，以及逻辑上改了什么。
