...
数据库三大范式是关系型数据库表设计的三条规范，主要目的是：减少数据冗余，避免插入/更新/删除异常，提高数据一致性。
...
第一范式 1NF：列不可再分
    ↓
第二范式 2NF：在 1NF 基础上，消除非主属性对主键的部分依赖
    ↓
第三范式 3NF：在 2NF 基础上，消除非主属性对主键的传递依赖
```
...
从严格范式角度看，`product_name` 可以根据 `product_id` 通过 JOIN 操作从商品表拿到商品名，不一定需要重复存储。但订单通常需要保留购买当时的商品快照。即使商品后来改名、改价，历史订单也不能跟着改变。因此保存：`product_name` 和 `purchase_price` 是合理的业务冗余。

**适用的场景**

1. 读多写少的业务：设置冗余字段从而减少 JOIN 操作，提升查询性能；
2. 需要数据快照的场景：比如订单里的商品名称、价格、下单时就固定了，不能后续变更；
...
> 注意：如果你要查询到某个字段没有索引，那么执行该字段的等值查询时，是需要全表扫描的。同时，执行该字段的范围查询时，也是需要全表扫描的，并不是说范围内的行都在相邻叶子节点上。因为只有设置了索引，该字段执行范围查询的时候，范围内的行才在相邻的叶子节点上。但是，全表扫描不需要回表。
...
#### 为什么 MySQL 采用 B+ 树作为索引结构？

> MySQL InnoDB 采用 B+ 树作为普通索引结构，主要是因为数据库按页进行磁盘 I/O，而 B+ 树分叉度高、树高度低，可以减少磁盘 I/O；非叶子节点只保存索引键和子节点指针，因此单页可以容纳更多索引项；同时叶子节点有序并相互连接，既适合等值查询，也非常适合范围查询、排序和顺序扫描。
>
> InnoDB 选择 B+ 树，最核心的原因不是“B+ 树查找复杂度比 B 树低”，两者本质上都是 O(logN)；真正关键的是数据库以页为单位进行磁盘 IO，B+ 树非叶子节点只保存索引，因此单个页面能容纳更多 key，树的分支更多、层数更低，从而减少 IO。同时叶子节点通过链表有序连接，非常适合数据库中大量的范围查询和顺序扫描。

InnoDB 的普通索引通常采用 B+ 树，主要因为磁盘 I/O 次数最少。数据库的数据存在磁盘上，磁盘随机读写比内存慢 10 万倍，所以索引结构的设计目标就是尽量减少磁盘访问次数。

数据库中最昂贵的不是比较，而是磁盘 I/O。如果有 1000 万条数据，使用普通二叉搜索树，一棵平衡二叉树大概需要 `log₂(10000000) ≈ 24`。也就是说，最坏可能需要访问二十多层节点。如果每个节点都分散在不同磁盘页上，就意味着可能发生二十多次磁盘 I/O，这个耗时是很长的。

B+ 树能够做到减少磁盘 I/O 次数，主要靠如下特性：

- B+ 树是多叉树，一个节点可以拥有几百上千个子节点，因此树很低，查询一条数据只需要较少的磁盘 I/O。

- B+ 树节点非常适合数据库“页”的存储方式。

  InnoDB 并不是一次从磁盘读取一条记录，而是按照页读取。InnoDB 默认页大小通常是 16 KB，所以设计索引结构时，最好让一个树节点正好对应一个数据页。读取一个节点，本质上就是进行一次页读取。如果将根节点缓存在内存中，那么 I/O 次数会更少。

- B+ 树的非叶子节点只保存索引键和子节点指针。这是 B+ 树相对 B 树的重要优势。
  ...
  因为 B+ 树非叶子节点不存完整数据，所以同样一个 16 KB 页面能保存更多索引键和指针。例如，B 树一个节点：`[主键 + 完整用户数据 + 指针]`，用户数据可能很大，一个页只能装几十条。而 B+ 树：`[主键 + 指针]`，可能一个页面就能保存上千个索引项。结果就是单个节点可以拥有更多子节点，从而树的分叉都更高，树的高度也就更低，那么磁盘 I/O 就更少。

- B+ 树特别适合范围查询。叶子节点有序并相互连接，适合范围查询。假设有索引：
  ...
  B+ 树叶子节点是有序的，而且叶子节点之间通常按顺序连接，MySQL 只需要：
  ...
  3. 扫描到 age > 30 时停止
   ...
   非常适合：`>, <, >=, <=, BETWEEN, ORDER BY, MIN(), MAX()`，这也是为什么数据库不能简单只使用 Hash。

- B+ 树所有数据都集中在叶子节点，查询性能更加稳定。

  B 树可能出现：`要找的数据恰好在根节点 → 1 次访问` 或 `要找的数据在叶子节点 → 3 次访问`，不同记录的查询路径长度可能不同。

  而 B+ 树真正的数据索引记录都集中在叶子层，所以大多数查询都会走 `根节点 → 中间节点 → 叶子节点`，路径长度比较稳定。
...
- B+ 树本身有序，可以直接帮助排序。例如：
  ...
  因为 B+ 树叶子节点本身就是 `18 → 20 → 21 → 25 → 30 → 35`，所以在条件合适时，可以直接按照索引顺序读取，避免额外排序。同样：

  ```sql
  SELECT MIN(age) FROM user;  # 可以直接找最左侧
  SELECT MAX(age) FROM user;  # 可以直接找最右侧
```

**MySQL 的 B+ 树中查询数据的全过程？**

> InnoDB 查询数据时，MySQL 优化器首先根据查询条件选择索引。进入 InnoDB 后，从 B+ 树根页开始，根据索引键逐层比较并找到对应子页面，直到定位到叶子页。页面如果已经存在 Buffer Pool 中就直接读取，否则从磁盘加载。对于聚簇索引，叶子节点直接保存完整数据行，因此找到叶子记录后即可返回；对于二级索引，叶子节点保存的是二级索引键和主键值，如果查询字段无法被索引覆盖，则还需要根据主键值再次查询聚簇索引，这个过程叫回表。范围查询则先通过 B+ 树定位范围起点，然后利用叶子节点之间的有序链表进行顺序扫描，直到超出查询范围。

在 MySQL InnoDB 中，B+ 树查询数据的全过程，本质上可以理解为：从根页开始，逐层定位到叶子页；如果是二级索引，再根据主键值回到聚簇索引查完整记录。
...
```
Server 层解析、优化 SQL，确定使用 PRIMARY 索引
 ↓
访问 B+ 树根节点
 ↓
定位中间节点
...
1. 确定使用哪个索引
   ...
   MySQL Server 层会先做：SQL 解析、语义分析，然后优化器生成执行计划，选择索引。
   ...
   优化器发现 id 是主键，于是选择 PRIMARY。如果执行：
...
2. 查 Buffer Pool
   ...
   InnoDB 不会每次都直接访问磁盘。它首先会判断需要的数据页是否已经在 Buffer Pool 中。因为 B+ 树节点本质上就是 InnoDB Page。默认页大小通常是 16 KB，所以可以理解成：B+ 树一个节点 ≈ 一个 InnoDB Page。查询过程实际上就是不断地：

   ```
   找到下一个 page_id
   ↓
   判断这个 Page 是否在 Buffer Pool
   ├─ 在 → 直接读取内存
   └─ 不在 → 从磁盘加载进 Buffer Pool
   ```

   因此实际数据库运行一段时间后，根节点和高层节点通常长期驻留在 Buffer Pool 中。
...
3. 从 B+ 树根节点开始查
   ...
   查询：`id = 1500`，先读取根页：`[1000 | 2000]`，比较发现：`1000 <= 1500 < 2000`。于是根据对应的子节点指针，找到下一层 Page。这里非叶子节点保存的主要内容可以简单理解为：
...
4. 继续查中间节点
   ...
   MySQL 继续比较：`id = 1500`，然后确定应该访问对应的下一级 Page。整个过程：根节点 → 内部节点 → 内部节点 → 叶子节点。

   因为 B+ 树分叉很多，所以通常只有很少几层。例如：
   ...
   即使数据有几千万条，B+ 树通常也不会特别高。

5. 最终找到叶子节点

   最终来到叶子页（一个 16 KB 的数据页），例如：
   ...
   然后在当前页内部找到：`id = 1500`，需要注意：InnoDB Page 内部并不是简单地从第一条记录一路扫描到最后一条。页中有 Page Directory，可以帮助快速定位记录所在的小范围，然后再在记录链中定位具体记录。可以粗略理解成：

   ```
   找到叶子 Page
        ↓
   利用 Page Directory 定位
        ↓
   在小范围记录中找到 id = 1500
   ```
...
- 主键查询找到叶子节点就直接结束
  ...
  因为 InnoDB 主键索引就是聚簇索引。聚簇索引的叶子节点存储：`主键 + 完整数据行`，所以：

  ```sql
  SELECT * FROM user WHERE id = 1500;
  ```

  查到主键索引叶子节点后，数据已经在那里了。不需要再去别的地方找。
...
- 如果是主键范围查询
  ...
  找到 id = 100 的叶子节点后，找到第一条 >= 100 的记录，沿 B+ 树叶子节点顺序向后扫描，一直扫描到 id > 200，然后停止。
...
- 如果使用的是二级索引
  ...
  表中有索引：`KEY idx_age(age)`，优化器可能选择：`idx_age`，这时候查询流程就不一样了。

  InnoDB 二级索引叶子节点保存的不是完整数据，而是：`二级索引字段 + 主键值`，比如：`age = 25, id = 1500`。所以第一步是先拿到 `id = 1500`，然后根据 `id = 1500` 再进行一次主键查询，拿到完整数据行。
...
- 如果是查询多条二级索引记录
  ...
  表中有索引：`KEY idx_age(age)`，优化器可能选择：`idx_age`。然后先定位 age = 20，沿二级索引叶子节点顺序扫描，得到 [20, 30] 对应的 id，再根据这些主键去聚簇索引找完整数据。

  > 这会产生大量回表。如果需要回表的记录太多，优化器可能认为：走二级索引 → 大量随机回表 → 成本很高。还不如：直接全表扫描。所以即使：age 有索引，MySQL 也不一定选择它。
...
- 如果是联合索引例如：
  ...
  B+ 树不是先建立一棵 `name` 树，再建立一棵 `age` 树。而是整个 Key：(name, age) 一起排序，排序规则就是 `先按照 name 排，name 一样时，再按照 age 排序`。

  ```sql
  SELECT * FROM user WHERE name = 'Alice' AND age = 20;
  ```
  
  就是从 (name, age) 这棵 B+ 树中定位：(Alice, 20)，这也是最左前缀原则形成的根本原因。

**为什么不用 Hash 作为索引结构？**

Hash 虽然查询很快，理论上单个数据查询速度接近 O(1)，但是 Hash 的问题是没有顺序，不支持支持范围查询，对于 `WHERE id > 10 AND id < 20` 这种 SQL，哈希表只能全表扫描，而 B+ 树顺着叶子节点走一遍就可以了。不支持排序，`ORDER BY` 得把数据捞出来再排，而 B+ 树的叶子节点天然有序，扫描出来就是排好的。

但是 InnoDB 也并非完全不使用 Hash，它有个自适应哈希索引。如果发现某个索引被频繁访问，会自动在内存里建一个哈希索引加速查询。

**为什么不用红黑树作为索引结构？**

红黑树也是平衡搜索树，但它本质上仍然是二叉树：每个节点最多两个子节点。因此如果数据非常多，B+ 树可能 3~4 层，红黑树可能几十层。数据库索引真正关心的是一次 I/O 能读取多少有效索引信息。

红黑树很适合内存中的：TreeMap、TreeSet，因为内存访问成本低。但磁盘数据库更希望：

```
一次读一个 16 KB Page -> 一个 Page 里放几百上千个索引项 -> 一次决定下一步去几百上千个方向中的哪一个
```
...
因为 B 树的非叶子节点也保存数据，而 B+ 树的非叶子节点仅用于保存索引，因此 B+ 树能够存放更多的 key，并且 B+ 树的叶子节点是有序连接的，更适合范围扫描。
...
#### 如果一个列既是单列索引，又是联合索引，单独查询它时会先用哪个？

MySQL 优化器是基于成本选择执行计划的：如果两个索引都能用于这条 SQL，它会估算哪个成本更低，就选哪个。
...
3. 优先选择区分度（一个字段的不同值有多少）高的字段建立索引（但同时也要考虑实际业务需求）。比如：性别、状态区分度低，手机号、身份证号区分度高。

4. 尽量使用联合索引，而不是堆很多单列索引。

   联合索引的注意事项：

   1）等值条件（`user_id = ?`）通常放在前面；

   2）考虑最左前缀原则；

   3）尽量设计覆盖索引；

   - 假设有 `INDEX idx_user_status(user_id, status)`，查询：

     ```sql
     SELECT user_id, `status` FROM orders WHERE user_id = 1001;
     ```

     需要的字段全部在索引里，因此可以直接从索引返回。不需要二级索引 → 主键 → 聚簇索引，也就是避免了回表。这就是覆盖索引。例如查询经常是：

     ```sql
     SELECT id, status, create_time FROM orders WHERE user_id = ?;
     ```

     可以考虑 `INDEX(user_id, status, create_time)`，因为 InnoDB 二级索引本身还包含主键 `id`，所以可能直接覆盖查询。但需要注意：不要为了覆盖索引，把十几个大字段全部塞进索引。否则索引会变得很大，导致：磁盘空间增加、B+ 树节点容纳的记录减少、Buffer Pool 命中率下降、树高度可能增加、INSERT / UPDATE 成本增加等。
   ...
   - 这个说法不能机械套用。例如：
     ...
     假设 user_id 区分度高，status 区分度低。那么 (user_id, status) 通常合理。但如果系统大量查询 `WHERE status = ?`，又大量查询：

     ```sql
     WHERE status = ? AND user_id = ?
     ```

     则 `(status,user_id)` 也可能更符合整体查询模式。所以联合索引字段顺序首先取决于 SQL 查询模式，其次才考虑字段选择性。

5. 大字段不要建索引。text、longtext 这种字段，索引占用空间大，加载到内存耗时，还可能把其他热点数据挤出 Buffer Pool，得不偿失。

6. 写多读少的表要慎重。如果修改频率远大于查询频率，索引带来的写入开销可能查过查询开销。

### 如何排查索引效果

建了索引不代表查询就会用，用了索引也不代表查询就快。MySQL 最终是否走索引，靠的是优化器的成本计算。优化器会评估走索引和全表扫描各自的 I/O 成本和 CPU 成本。有时候全表扫描确实成本低，比如表就几百行数据，走索引还不如直接扫。

排查索引效果最直接的办法就是用 EXPLAIN 命令，在 SQL 前面加上 EXPLAIN，就能看到 MySQL 选择的执行计划。例如：
...
4. 串行化
   ...
   隔离级别最高。数据库让并发事务尽量按照串行顺序执行：事务 A 执行完成，事务 B 再执行。
...
- 当前读读取的是数据库中最新已提交的数据版本。为了保证数据一致性，它在读取的同时必须对数据加锁。

快照读和当前读指的是怎样读数据，记录锁、间隙锁、临键锁指的是当前读是怎样加锁。
...
2. Redo Log 保证崩溃恢复和持久性
   ...
   数据库重启时，InnoDB 可以读取 Redo Log，将尚未完整写入数据文件的修改重新执行。官方文档说明，Redo Log 用于崩溃恢复；意外关闭前没有完成数据文件更新的修改会在初始化期间自动重放。这就是常说的 WAL（Write-Ahead Logging）：先写日志，再允许对应数据页稍后刷盘。
...
长事务指一个事务从开始到 `COMMIT / ROLLBACK` 持续时间很长。

在 MySQL InnoDB 中，长事务的问题不只是“锁时间长”。即使是一个只读长事务，也可能因为长期持有旧的 Read View，阻碍 Undo 历史版本清理。
...
- 锁竞争严重，容易引发连锁反应。事务持有行锁、间隙锁的时间越长，其他事务等待的时间就越久。业务线程被阻塞后，连接池被占满，上游服务超时，搞不好就雪崩了。
- 死锁风险增加。事务执行时间越长，持有的锁越多，和其他事务产生循环等待的概率就越大。InnoDB 虽然有死锁检测机制，但检测本身也消耗资源，而且检测到了只能回滚其中一个事务，业务还是要受影响。
- undo log 膨胀。InnoDB 的MVCC 机制需要保留事务开始时的数据版本，长事务不提交，这个版本链就一直不能清理。一个跑了几个小时的事务，可能导致 undo log 从几百 MB 涨到几十 GB，磁盘空间告警不说，回滚段查询的性能也会变差。
- 主从延迟。主库执行一个大事务要 10 分钟，binlog 传到从库后，从库也要重放 10 分钟。这期间主从数据不一致，读从库的业务拿到的都是脏数据。
- 回滚代价大。事务执行了 20 分钟，突然报错要回滚，之前的活全白干了。更惨的是回滚本身也要时间，可能又要等 20 分钟。
...
3. 临键锁，Next-Key Lock，是：`记录锁 + 间隙锁`。
   ...
   它锁定一个左开右闭区间。假设索引值为：10、20、30。对应的临键锁区间大致是：
...
1. 意向锁（Intention Locks），一种表级锁，由 InnoDB 自动维护，用户无法操作。用于表示：某个事务准备或已经对表中的某些行加锁。主要有：意向共享锁 IS（Intention Shared Lock）和意向排他锁 IX（Intention Exclusive Lock）。
   ...
   不同事务的 `IX` 和 `IX` 可以兼容，因为两个事务可以分别修改不同的数据行。
...
2. 插入意向锁（Insert Intention Lock），一种特殊的间隙锁，表示事务准备在某个索引间隙中插入记录。
   ...
   > 有了插入意向锁能够让间隙中不同位置并发插入，为什么还需要间隙锁，要知道间隙锁锁的是整个区间？
   >
   > 如果只有插入意向锁，没有间隙锁：就无法防止间隙插入的幻读问题。
   > 如果只有间隙锁，没有插入意向锁：虽然能够解决幻读问题，但是间隙的插入是串行的，不能并发执行，效率太低。
...
3. AUTO-INC 锁
   ...
   一般情况下，普通单行插入能够高效分配自增值；批量插入、`INSERT ... SELECT` 等语句可能采用更严格的协调方式。自增值通常保证唯一递增，但不保证绝对连续。例如事务获取了自增值后回滚，这个自增值通常不会重新使用，因此可能产生间断。
...
1. 悲观锁：假设并发冲突很可能发生，因此先加锁，再操作数据。例如：
   ...
   > 上述是悲观锁，select ... for update 已经给 id = 1 索引加了排他锁了，为什么 update 还可以修改 id = 1 的数据？
   >
   > 因为这个排他锁是当前事务自己持有的。锁的作用是阻止“其他事务”并发修改，不是阻止持锁事务自己修改。
...
- 表级锁、页级锁、行级锁说的都是锁的粒度。
- 共享锁（S 锁，允许持有者读取）和排他锁（X 锁，允许持有者修改或删除）说的都是锁的兼容性。
...
Redo Log，即重做日志，记录 InnoDB 数据页的物理修改信息。例如，执行某句 SQL 后，某个数据页的某个位置发生了什么修改。主要用于数据库异常宕机后的崩溃恢复，保证事务的持久性。
...
事务提交时，优先确保必要的 Redo Log 按配置写入磁盘；脏页可以由后台线程稍后异步刷入数据文件。

因此，即使事务刚提交，数据库就异常退出，重启后，InnoDB 可以从最近的 Checkpoint 开始重放 Redo Log，恢复没有完整写入数据文件的修改。

注意：Redo Log 是循环写的，空间固定，写满了就得等 checkpoint 推进才能继续。

### Undo Log

Undo Log，即回滚日志，记录的是能够将数据恢复到修改前版本所需的信息，属于逻辑日志。例如执行 `update` 后，会保存恢复旧版本所需的信息，从而支持事务回滚。同时 Undo Log 形成的历史版本链也用于 MVCC。
...
事务提交后，Undo Log 不一定立即清理。如果其他事务仍然持有旧的 Read View，它们可能还需要读取旧版本，因此相应 Undo 记录不能删除。等没有事务再需要这些历史版本后，Purge 线程才会进行清理。这也是长事务可能造成问题的原因：长事务持有旧 Read View，历史版本不能清理，Undo Log 堆积，版本链变长，从而空间和查询成本增加

### Binlog

Binary Log，记录数据库发生的数据变更事件，属于逻辑日志。支持 `STATEMENT`、`ROW`、`MIXED` 格式。例如，创建表、修改表结构、插入数据、修改数据、删除数据等操作。`STATEMENT` 记录执行的 SQL，`ROW` 记录具体哪些行发生了怎样的数据变化。
...
- 数据恢复：通过备份 + binlog 恢复到某个时间点
  ...
  所以说在数据恢复中，binlog 的作用就是：把数据库从某个备份时刻，恢复到之后的某个指定时间点。比如：

  ```
  凌晨 2:00 做了一次全量备份
  2:00 ~ 10:00 数据库一直有 INSERT / UPDATE / DELETE
  10:05 误执行 DELETE FROM user;
  ```

  如果你只有凌晨 2 点的全量备份，那么最多只能恢复到凌晨 2 点。但如果同时保留了 Binlog：

  ```
  先根据全量备份将数据恢复到 02:00 的状态，然后重放 02:00 ~ 10:04 的 Binlog，最终将数据库恢复到误删之前
  ```

  这就是 Binlog 在数据恢复中的作用。
...
MySQL 主从同步，也叫主从复制。更准确的术语是：Source（源库），通常称主库；Replica（副本库），通常称从库。
...
它解决的核心问题是：单库容量过大、单表数据过多、单库读写压力过高、单机性能达到瓶颈。

可以先记住：分库，把数据拆到不同数据库；分表，把数据拆到不同数据表。
...
- Server 层负责连接管理、SQL 解析、查询优化等操作；
- 存储引擎层负责数据的实际存取。

一条 SQL 语句从发出到拿到结果，大概经过如下步骤：

1. 客户端先通过 TCP 跟 MySQL 建立连接，**连接器**负责验证账号密码和权限，确认你能对哪些库表做什么操作；
2. 老版本 MySQL 会先查查询缓存，看有没有一模一样的 SQL 缓存过结果。但 MySQL 8.0 取消了该功能，因为表一更新缓存就全失效，命中率太低；
3. **分析器**对 SQL 做词法分析和语法分析。先把字符串拆成一个个 token，识别出 SELECT、表名、列名、WHERE 条件这些元素；再按语法规则检查 SQL 写得对不对，最后生成一棵抽象语法树；
4. **优化器**拿到语法树后，决定用哪个索引、多表 JOIN 时先查哪张表、子查询要不要改写成 JOIN，最终生成一个执行计划；
5. **执行器**按照执行计划，调用存储引擎的接口一行一行地读数据，做条件过滤，最后把结果集返回给客户端。

连接器建连、分析器解析、优化器决策、执行器调度、存储引擎存取，这就是完成的流程。



## 待整理

[MySQL 在设计表（建表）时需要注意什么？ - 面试鸭 | 2026最新面试题+详细答案解析](https://www.mianshiya.com/question/1854714622329430017)
